...
function sanitizeSegment(s: unknown): string {
...
type EnvOverlay = { overlay: Record<string, string>; redactValues: string[]; keys: string[] };

async function loadEnvOverlayFromRepoDotenv(repoRoot: string): Promise<EnvOverlay> {
...
async function redactFileInPlace(filePath: string, redactValues: string[]): Promise<void> {
...
function createRedactingTransform(redactValues: string[]): stream.Transform {
...
function spawnAndTee({ cwd, ws, cmd, args, env }: SpawnAndTeeArgs): Promise<number> {
  return new Promise((resolve) => {
    ...
    child.on('error', (err) => {
      try {
        ws.write(
          `\n[spawn error] ${cmd}: ${String(err && (err as Error).message ? (err as Error).message : err)}\n`,
        );
      } catch {
        // ignore
      }
...
async function appendMarker(ws: stream.Writable, text: string): Promise<void> {
  ws.write(`\n===== ${text} =====\n`);
  // Let piping flush so markers don't interleave with output too badly.
  await sleep(5);
}

type Step = { exitCode: number; wallTimeMs: number };

async function runStep(
  args: { ws: stream.Writable; label: string } & Omit<SpawnAndTeeArgs, 'ws'>,
): Promise<Step> {
  const start = Date.now();
  await appendMarker(args.ws, args.label);
  const exitCode = await spawnAndTee({
    cwd: args.cwd,
    ws: args.ws,
    cmd: args.cmd,
    args: args.args,
...
function buildChildEnv(args: {
  ...
  unsafeForwardAllEnv: boolean;
}): NodeJS.ProcessEnv {
  if (args.unsafeForwardAllEnv) {
    return { ...args.baseEnv, ...args.overlay };
  }
  ...
  for (const [k, v] of Object.entries(args.baseEnv || {})) {
    if (v == null) continue;
    if (allowExact.has(k)) {
      childEnv[k] = v;
      continue;
  ...
  for (const [k, v] of Object.entries(args.overlay || {})) {
    if (v == null) continue;
    childEnv[k] = v;
  }
...
async function main(): Promise<void> {
  ...
  let safeSkill: string;
  let safeStarter: string;
  try {
    safeSkill = assertSafeSegment(skill, '--skill');
    safeStarter = assertSafeSegment(starter, '--starter-project');
  } catch (err) {
    console.error(String(err && (err as Error).message ? (err as Error).message : err));
    process.exit(2);
  }
    ...
    ...(acceptance.lines.length > 0
      ? [
          ...
          ...(acceptance.acceptancePath
            ? [`(source: ${path.relative(repoRoot, acceptance.acceptancePath)})`]
            : []),
        ]
      : []),
    '',
  ...
  if (await exists(diffStatPath)) console.log(`Diff stat: ${diffStatPath}`);
