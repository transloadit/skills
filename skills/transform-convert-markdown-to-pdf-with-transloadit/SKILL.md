...
description: Convert a local Markdown file to a sibling PDF via the official `@transloadit/node` CLI. Use when the user wants a `.md` file rendered as a `.pdf`, especially from an agent session or local repo. Resolve `TRANSLOADIT_KEY` and `TRANSLOADIT_SECRET` from the current environment first, then from a nearby `.env` file if needed.
---
